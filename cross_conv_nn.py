...
def get_base_model(input_len,fsize ,feature_num):
    '''Base network to be shared (eq. to feature extraction).
    '''
    input_seq = Input(shape=(input_len, feature_num))
    nb_filters = 50
    convolved = Conv1D(nb_filters, filter_length=fsize, border_mode="same", activation="relu",)(input_seq)
    # convolved = Dropout(0.7)(convolved)
...
def get_full_model(epoch_len,feature_num):
    # input_quarter_seq = Input(shape=(int(epoch_len/4), channel_number))
    # input_half_seq = Input(shape=(int(epoch_len/2), channel_number))
    ...
    opt = RMSprop(lr=0.0005, clipvalue=10**6)
    model.compile(loss='categorical_crossentropy',metrics=['accuracy'], optimizer=opt)
    return model
def to_onehot(y):
